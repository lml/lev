...
module Lev
  class BackgroundJob
    ...
    # If no such ID exists in the store,
    # creates a job with 'unknown' status and sets it in the store
    def self.find!(id)
    ...
    # Finds the job with the specified ID and returns it.
    # If no such ID exists in the store, returns nil.
    def self.find(id)
      raise(ArgumentError, "`id` is nil") if id.nil?

      attrs = { id: id }

      existing_job_attrs = fetch(job_key(id))
    ...
    def failed!(exception = nil)
      if exception.is_a?(Lev::FatalError)
        # noop
      elsif exception.nil?
        set(status: 'failed')
      else
        @errors << { is_fatal: true,
                     code: :exception,
                     message: exception.message,
                     data: exception.backtrace.first }
        set(errors: @errors)
        set(status: 'failed')
      end
    end
    ...
    def save_standard_values
      set({ id: id, status: status, progress: progress, errors: errors })
    end
    ...
    def get_dynamic_variable(name)
      has_dynamic_variable?(name) ? instance_variable_get("@#{name}") : nil
    end
    ...
    def self.fetch(job_key)
      ...
      fetched.nil? ? nil : JSON.parse(fetched).stringify_keys!
    end
